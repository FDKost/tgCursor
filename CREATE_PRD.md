...
\```typescript
export async function createFeature(input: FeatureInput): Promise<FeatureResult> {
  const validated = validate(input);
  const result = await repository.save(validated);
  return toDTO(result);
}
\```
...
\```
User action → UI form submission → Backend validation → Database write → Success confirmation
\```
...
| Unauthorized | "You don't have permission" | Redirect to login |
...
| Security | No exposed secrets or PII in logs |
...
### Step 1 — Data Layer
- implement database changes · create migration · add unit tests  
**Validation:** run project validation pipeline.

### Step 2 — Business Logic
- implement service layer · add input validation · write tests  
**Validation:** run project validation pipeline.

### Step 3 — API / Interface
- implement endpoint · add integration tests · connect UI if applicable  
**Validation:** run project validation pipeline.
...
### Allowed
- run tests · update files related to this feature · reuse existing patterns · create new files within defined scope

### Requires Approval
- introducing new dependencies · modifying shared interfaces · altering database schema beyond defined changes · changing public API contracts

### Forbidden
- commit secrets · remove tests · modify historical migrations · bypass type safety rules · push directly to main/production branch
...
| Term | Meaning |
|------|---------|
| Agent | Autonomous development agent executing tasks defined in this PRD |
| PRD | Product Requirements Document |

### Related Resources

- design document · technical specification · related PRDs · PROJECT.md